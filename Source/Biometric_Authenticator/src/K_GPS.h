...
This is K_GPS C code.
This code filters the GPRMC string.
This code prints Latitude, Longitude, K_date and Time only.
...
double dt;				//gets the K_date
int hr, min, sec;		//displays the time
int dd, mm, yr;			//displays the K_date
int L_c = 0;


/*Converts Degrees to Decimal Degrees*/
double K_deg_conv(double coord) {
...
void K_UART() {
	system("config-pin P9.26 uart >/dev/null 2>&1");
	L_file = open("/dev/ttyO1", O_RDWR | O_NOCTTY);

	struct termios options;               //The termios structure is vital
	tcgetattr(L_file, &options);        //Sets the parameters associated with L_file
	options.c_cflag = B9600 | CS8 | CREAD | CLOCAL;
	options.c_iflag = IGNPAR;    //ignore partity errors, CR -> newline
	tcflush(L_file, TCIFLUSH);          //discard file information not transmitted
	tcsetattr(L_file, TCSANOW, &options);  //changes occur immmediately
}
...
void K_GPS() {
	...
	while (L_c != 10) {
		do {
			...
			int characters = read(L_file, (void*) &recieve, 256); //reception of the string
			...
			if (p != NULL) {
				lat = K_deg_conv(atof(p));//converts string to double and then to decimal coordinates
			...
			if (p != NULL) {
				lon = K_deg_conv(atof(p));//converts string to double and then to decimal coordinates
			...
			if (p != NULL) {
				...
				K_date(dt);					//K_date conversion
				//printf("K_date %d/%d/%d\n", dd, mm, yr);
				p = strchr(p, ',') + 1;			//Skips K_date
				//printf(" %s", p);
			}
	...
	printf("K_date %02d/%02d/%d\n", dd, mm, yr);		//prints the K_date
	close(L_file);
	sleep(3);
	//KG_webpage();
	}
...
void KG_webpage() {
	/*------------------------------------------------------*/
		...
		char buffer2[500];				//K_date buffer string
		char plink[100] = " <a href=\"https://www.google.com/maps/place/";

		sprintf(buffer2, "%s %d/%d/%d", buf, dd, mm, yr);//converts K_date to string
		//printf("\n%s\n",buffer2);
		strncat(buffer2, plink, 58);			//appends K_date to plink. (date_link)
		//printf("\n%s\n",buffer2);
		...
		strncat(buffer2,
				"\" target=\"_blank\" class=\"button2\" >K_GPS LOCATION</a></p> ",
